# similar to past implementations
...
resource "azurerm_application_gateway" "web_ag" {
    ...
    #sku block, standard_v2
    sku {
        name     = "Standard_v2"
        tier     = "Standard_v2"
        # capacity = 2 #can use autoscaling instead
    }
    ...
    ### app1 configs
    # backend address pool for listener
    backend_address_pool {
      name = local.backend_address_pool_name_app1
    }
    # http_settings for listener
    backend_http_settings {
      name = local.http_setting_name_app1
      cookie_based_affinity = "Disabled"
      #  path = "/app1/" #override path, if backend listening on path can override root path
      port = 80
      protocol = "Http"
      request_timeout = 60
      probe_name = local.probe_name_app1 # need to associate after probe made
    }
    probe {
    ...
    # associatie the rule to the listener then http setting and backend pool
    # rule-1
